...
func GetInfo(urlInit string) (title string, info string, epiCount int) {
	...
	c2.OnHTML(".section-wrap", func(e *colly.HTMLElement) {
		//$("strong.lt").text() //标题
	...
	//UserAgent PC
	c2.UserAgent = "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.142 Safari/537.36"
	c2.Visit(urlInit)
...
func StartCollectV2() {
	...
	//获取标题，信息，集数
	urlInit := http_proxy + domain + "/drama/HC1qLS8="
	title, info, epiCount := GetInfo(urlInit)
	//写到本地文本
	fileName := Prefix + title + ".txt"
	CollyUtils.AppendToFile(fileName, info)
	//init
	urlEpisode := http_proxy + domain + "/drama/HC1qLS8=" + "/episode"
